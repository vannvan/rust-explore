...
export const useGlobalExportManager = () => {
  ...
  const docMapRef = useRef<Map<string, TreeNode & { bookSlug: string }>>(new Map())

  // 创建导出任务的公共函数
  const createExportTask = useCallback(
    (doc: TreeNode, bookSlug: string, isBatch: boolean = false): ExportTask => {
  ...
  const addTasksToQueue = useCallback(
    (newTasks: ExportTask[], docs: TreeNode[], bookSlug: string) => {
      // 存储 TreeNode 的映射，并添加 bookSlug 信息
  ...
  const processQueue = useCallback(async () => {
    ...
    console.log('开始处理全局导出队列，当前队列长度:', taskQueueRef.current.length)
  ...
  const addExportTask = useCallback(
    (doc: TreeNode, bookSlug: string) => {
      ...
      const task = createExportTask(doc, bookSlug, false)
      console.log('创建的任务:', task)

      // 使用公共函数添加任务
      addTasksToQueue([task], [doc], bookSlug)
    },
  ...
  const addBatchExportTasks = useCallback(
    (docs: TreeNode[], bookSlug: string) => {
      console.log('=== addBatchExportTasks 被调用 ===')
      console.log('接收到的文档数量:', docs.length)
      console.log('当前bookSlug:', bookSlug)

      const newTasks = docs.map((doc) => createExportTask(doc, bookSlug, true))
      console.log('创建的批量任务数量:', newTasks.length)

      // 使用公共函数添加任务
      addTasksToQueue(newTasks, docs, bookSlug)
    },
    [createExportTask, addTasksToQueue]
  )
